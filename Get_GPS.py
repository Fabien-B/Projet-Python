...
class GPScoord(QtCore.QObject):
    ...
    def __init__(self, cache):
        """
        Set the locator and initialize the number
        of successes and the number of tries
        """
    ...
    def find(self, adresse, name='unknow Name', i=0, j=0):
        ...
        try:
            ...
            if loc == None or loc[0].coordinates == (43.604652, 1.444209):
                ...
                self.success += 1
                self.timestried = 0
                self.succesSignal.emit("Adresse trouvée : {}       "
                                       "{}/{}".format(name, i, j))
    ...
    def findall(self, eqpmtlist=None):
        ...
        for i in range(len(self.equipmentlist)):
            if not self.equipmentlist[i].coords:
    ...
    def get_random(self, eqpmtlist):
        """
        Find random coordinates for each equipment in list (ONLY FOR DEBUG)
        :param eqpmtlist: The equipment list
        :return: The new equipment list with the random coordinates
        """
        for i in range(len(eqpmtlist)):
            if not eqpmtlist[i].coords:
    ...
    def setproxy(self, list):
        """
        Used to set the proxy for the geolocator
        :param list: The list containing the proxy parameters
